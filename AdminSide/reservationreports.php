...
try {
    ...
    // Query to get reservations for the specific branch
    $stmt = $pdo->prepare("
        SELECT 
            r.ReservationID,
            r.UserID,
...
<head>
    ...
    <title>Reservation Reports - <?php echo htmlspecialchars($loggedInBranch); ?></title>
...
<body>
    ...
    <div class="main">
        <div class="topbar">
            <div class="logo">
                <i class="fas fa-chart-bar"></i> Reservation Reports
            </div>
        ...
        <div class="main-container">
            ...
            <div class="page-header">
                <div class="header-content">
                    <div class="header-left">
                        <p class="header-subtitle">Comprehensive analytics and reporting dashboard</p>
                    </div>
            ...
            <div class="stats-grid">
                ...
                <div class="stat-card">
                    <div class="stat-header">
                        <div class="stat-label">Paid Reservations</div>
                        <div class="stat-icon">✅</div>
                    </div>
                        ...
                        <tbody id="tableBody">
                            <tr>
                                <td colspan="9" class="no-data">
                                    ...
                                    <div class="no-data-text">No Reservation Records</div>
                                    <div class="no-data-subtext">Apply filters to view reservation data</div>
                                </td>
                            </tr>
                        </tbody>
                    </table>
    ...
    <div class="print-only">
        <div class="print-header">
            <h1 class="print-title">📊 Reservation Reports & Analytics</h1>
        ...
        <div class="print-stats">
            <div class="print-stat">
                <div class="print-stat-label">Total Reservations</div>
                <div class="print-stat-value" id="printTotalCount">0</div>
            </div>
            <div class="print-stat">
            ...
            <div class="print-stat">
                <div class="print-stat-label">Paid</div>
                <div class="print-stat-value" id="printPaidCount">0</div>
            </div>
            <div class="print-stat">
                <div class="print-stat-label">Pending</div>
                <div class="print-stat-value" id="printPendingCount">0</div>
            </div>
        </div>
    ...
    <script>
        ...
        function applyFilters() {
            ...
            filteredData = dataToFilter.filter(res => {
                ...
                // Date filtering
                if (filterType === 'today') {
                    dateMatch = resDate.toDateString() === today.toDateString();
                } else if (filterType === 'month') {
                    dateMatch = resDate.getMonth() === today.getMonth() && 
                               resDate.getFullYear() === today.getFullYear();
                } else if (filterType === 'year') {
                    dateMatch = resDate.getFullYear() === today.getFullYear();
                } else if (filterType === 'specific-month') {
                    const selectedMonth = document.getElementById('monthSelect').value;
                    if (selectedMonth) {
                        const [year, month] = selectedMonth.split('-');
                        dateMatch = resDate.getMonth() === parseInt(month) - 1 && 
                                   resDate.getFullYear() === parseInt(year);
                    }
                } else if (filterType === 'custom') {
                    ...
                    if (startDate && endDate) {
                        ...
                        dateMatch = resDate >= start && resDate <= end;
                    }
                }

                // User filtering - filter by UserID
                const userMatch = !userFilter || (res.UserID && res.UserID.toString() === userFilter);

                // Payment status filtering
                const statusMatch = !paymentStatus || res.PaymentStatus === paymentStatus;
            ...
            if (filteredData.length === 0) {
                tbody.innerHTML = `
                    <tr>
                        <td colspan="9" class="no-data">
                            <div class="no-data-icon">📊</div>
                            <div class="no-data-text">No Reservation Records Found</div>
                ...
                printTbody.innerHTML = `
                    <tr>
                        <td colspan="9" style="text-align: center; padding: 20px; color: #999;">No data to display</td>
                    </tr>
                `;
                return;
            }

            const tableRows = filteredData.map(res => `
                <tr>
                    <td><strong>${res.ReferenceNumber}</strong></td>
                    <td>${new Date(res.Date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</td>
                    <td>${res.Time}</td>
                    <td>${res.Fname} ${res.Lname}</td>
                    <td><strong>${res.PlateNo}</strong></td>
                    <td>${res.Brand}</td>
                    <td><span class="badge badge-${res.PaymentMethod}">${res.PaymentMethod.toUpperCase()}</span></td>
                    <td><span class="badge badge-${res.PaymentStatus}">${res.PaymentStatus.toUpperCase()}</span></td>
                    <td><strong>₱${parseFloat(res.Price).toFixed(2)}</strong></td>
                </tr>
            `).join('');

            tbody.innerHTML = tableRows;
            printTbody.innerHTML = tableRows;
        ...
        function updateStats() {
            ...
            const totalRevenue = filteredData.reduce((sum, res) => sum + parseFloat(res.Price), 0);
            const paid = filteredData.filter(res => res.PaymentStatus === 'paid' || res.PaymentStatus === 'verified').length;
            const pending = filteredData.filter(res => res.PaymentStatus === 'pending').length;
            ...
            // Update print stats
            document.getElementById('printTotalCount').textContent = total;
            ...
            document.getElementById('printPaidCount').textContent = paid;
            document.getElementById('printPendingCount').textContent = pending;
