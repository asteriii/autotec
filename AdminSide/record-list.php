...
$count_sql = "SELECT COUNT(*) as total FROM records r $search_condition";
$count_stmt = $pdo->prepare($count_sql);
$count_stmt->execute($params);
$total_records = $count_stmt->fetch(PDO::FETCH_ASSOC)['total'];
$total_pages = ceil($total_records / $records_per_page);

// Fetch records with vehicle type information and branch filter
...
// Process receipt paths for each records
foreach ($recordss as &$records) {
    $receiptPath = $records['PaymentReceipt'] ?? '';
    ...
    if ($hasReceipt) {
        ...
        $records['_hasReceipt'] = true;
        $records['_receiptExists'] = $receiptExists;
        $records['_receiptPath'] = $cleanPath;
        $records['_fullPath'] = $fullPath;
        
        // Debug logging
        error_log("Records ID {$records['RecordID']}: Receipt Path = {$receiptPath}, Clean Path = {$cleanPath}, Full Path = {$fullPath}, Exists = " . ($receiptExists ? 'YES' : 'NO'));
    } else {
        $records['_hasReceipt'] = false;
        $records['_receiptExists'] = false;
        $records['_receiptPath'] = '';
        $records['_fullPath'] = '';
    }
}
unset($records); // Break reference
?>
...
<head>
    ...
    <title>Records - AutoTec Admin</title>
...
<body>
    ...
    <div class="main">
        <div class="topbar">
            <div class="logo">
                <i class="fas fa-check-circle"></i> Records List
            </div>
        ...
        <div class="content">
            ...
            <?php if (empty($recordss)): ?>
                <div class="no-records">
                    <i class="fas fa-calendar-times"></i>
                    <h3>No recorded appointments found</h3>
                    <p>There are currently no recorded appointments matching your criteria.</p>
                </div>
            <?php else: ?>
                <?php foreach ($recordss as $records): ?>
                    <?php
                    // Get processed receipt data
                    $hasReceipt = $records['_hasReceipt'];
                    $receiptExists = $records['_receiptExists'];
                    $receiptPath = $records['_receiptPath'];
                    
                    // Payment method checks
                    $paymentMethod = strtolower($records['PaymentMethod'] ?? '');
                    $isGcash = $paymentMethod === 'gcash';
                    $isOnsite = $paymentMethod === 'onsite' || $paymentMethod === 'cash';
                    ?>
                    <div class="records-card">
                        <div class="card-header">
                            <div class="records-id">ID: <?php echo $records['RecordID']; ?></div>
                            <?php
                        // Determine status badge text and class
                        $statusText = '';
                        $statusClass = '';

                        if (isset($records['RecordStatus'])) {
                            if (strtolower($records['RecordStatus']) === 'no show') {
                                $statusText = 'No Show';
                                $statusClass = 'status-no-show'; // new CSS class
                            } else if (strtolower($records['RecordStatus']) === 'accomplished') {
                                $statusText = 'Accomplished';
                                $statusClass = 'status-accomplished'; // new CSS class
                            } else {
                                $statusText = htmlspecialchars($records['RecordStatus']); // fallback
                        ...
                        <div class="card-content">
                            <div class="info-section">
                                ...
                                <p><strong>Name:</strong> <?php echo htmlspecialchars($records['Fname'] . ' ' . $records['Mname'] . ' ' . $records['Lname']); ?></p>
                                <p><strong>Phone:</strong> <?php echo htmlspecialchars($records['PhoneNum']); ?></p>
                                <p><strong>Email:</strong> <?php echo htmlspecialchars($records['Email']); ?></p>
                                <p><strong>Address:</strong> <?php echo htmlspecialchars($records['Address']); ?></p>
                            </div>
                            
                            <div class="info-section">
                                <h4><i class="fas fa-car"></i> Vehicle Details</h4>
                                <p><strong>Brand:</strong> <?php echo htmlspecialchars($records['Brand']); ?></p>
                                <p><strong>Plate No:</strong> <?php echo htmlspecialchars($records['PlateNo']); ?></p>
                                <p><strong>Vehicle Type:</strong> <?php echo htmlspecialchars($records['VehicleTypeName'] ?? 'Unknown'); ?></p>
                                <p><strong>Category:</strong> <?php echo htmlspecialchars($categories[$records['CategoryID']] ?? 'Unknown'); ?></p>
                            </div>
                            
                            <div class="info-section">
                                <h4><i class="fas fa-calendar-alt"></i> Appointment Details</h4>
                                <p><strong>Date:</strong> <?php echo date('M d, Y', strtotime($records['Date'])); ?></p>
                                <p><strong>Time:</strong> <?php echo date('g:i A', strtotime($records['Time'])); ?></p>
                                <p><strong>Branch:</strong> <?php echo htmlspecialchars($records['BranchName'] ?? 'N/A'); ?></p>
                                <p><strong>Payment:</strong> <?php echo strtoupper($records['PaymentMethod'] ?? 'N/A'); ?>
                                    <?php if ($isGcash): ?>
                                ...
                                <div class="price-tag">
                                    ₱<?php echo number_format($records['VehiclePrice'] ?? 0, 2); ?>
                                </div>
                            </div>
                        </div>
                            ...
                            <?php if ($isGcash && $hasReceipt && $receiptExists): ?>
                                <button class="btn view-receipt-btn" 
                                        ...
                                        onclick="viewReceipt('/<?php echo htmlspecialchars($receiptPath); ?>', 
                                                             '<?php echo htmlspecialchars($records['ReferenceNumber'] ?? 'N/A'); ?>', 
                                                             '<?php echo htmlspecialchars($records['PaymentMethod']); ?>', 
                                                             '<?php echo htmlspecialchars($records['PaymentStatus'] ?? 'pending'); ?>',
                                                             '<?php echo htmlspecialchars($records['Fname'] . ' ' . $records['Lname']); ?>',
                                                             '₱<?php echo number_format($records['VehiclePrice'] ?? 0, 2); ?>')">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            console.log('Record list page loaded');
            console.log('Total Record on page:', <?php echo count($recordss); ?>);
        });
